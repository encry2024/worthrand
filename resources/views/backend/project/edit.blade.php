...
@section('content')
{{ html()->modelForm($project, 'PATCH', route('admin.project.update', $project->id))->class('form-horizontal')->open() }}
    ...
    <div class="card">
            ...
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        ...
                        <small class="text-muted">{{ __('labels.backend.projects.edit', ['project' => $project->name]) }}</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
                        ...
                        <div class="col-md-10">
                            <select name="customer" id="customer-dropdown" class="form-control chosen-select">
                                ...
                                @foreach ($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ $project->customer_id == $customer->id ? "selected" : "" }}>{{ $customer->name }}  {{ $customer->trashed() ? " | Deleted" : "" }}</option>
                                @endforeach
                            </select>
                        </div><!--col-->
                    </div><!--form-group-->
        ...
        <div class="card-footer clearfix">
            <div class="row">
                <div class="col">
                    {{ form_cancel(route('admin.project.show', $project->id), __('buttons.general.cancel')) }}
                </div><!--col-->
